...
namespace EPicture
{
    public sealed partial class SecondPage : Page
    {
        ...
        {
            Epi = e.Parameter as EPicture_Imgur;
            textBlock.Text = "Connected as " + Epi.account_username;
        }
        ...
        private async void buttonUpload_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (file != null)
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", Epi.access_token);
                    MultipartFormDataContent data = new MultipartFormDataContent();
                    data.Add(content, "image", "image");
                    data.Add(content, "name", file.Name);
                    data.Add(content, "description", "Another Epicture upload =)");
                    ...
                    {
                        List<ImgurImage> images = await Epi.getAllUserImages();
                        LoadImage(images);
                    }
                }
            }
        ...
        private async void buttonGalHot_Click(object sender, RoutedEventArgs e)
        {
            ...
            List<ImgurImage> images = await Epi.getGallery();
            LoadImage(images);
        }

        private async void buttonDelete_Click(object sender, RoutedEventArgs e)
        {
            await Epi.RemoveImage(selected_imageId);
            List<ImgurImage> images = await Epi.getAllUserImages();
            LoadImage(images);
        }
        ...
        private async void buttonGalFav_Click(object sender, RoutedEventArgs e)
        {
            ...
            buttonDelete.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
            searchBox.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
            List<ImgurImage> images = await Epi.getFavorites();
            LoadImage(images);
        }
        ...
        {
            List<ImgurImage> images = await Epi.SearchGallery(searchBox.Text);
            LoadImage(images);
        }

        private async void buttonFav_Click(object sender, RoutedEventArgs e)
        {
            await Epi.AddFavorite(selected_imageId);
        }
    }
}
